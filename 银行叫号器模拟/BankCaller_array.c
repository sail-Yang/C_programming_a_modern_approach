...
#define MAX_CUSTOMERS 500			//对列最多排多少个人
...
{
	int queue_len = (Q->rear - Q->front+MAX_CUSTOMERS)%MAX_CUSTOMERS;
	if(Q->front == (Q->rear+1)%MAX_CUSTOMERS)
		printf("此队列已满");
	Q->customer[Q->rear].ArrivalTime = ArrivalTime;
	Q->customer[Q->rear].Duration = Duration;
	Q->customer[Q->rear].num = CustomerNum;
...
void DeQueue(Person *person_ptr)
{
	...
	*person_ptr = Q->customer[Q->front];	//提取这个客户的信息
...
void PrintQueue()
{
	int queue_len = (Q->rear - Q->front+MAX_CUSTOMERS)%MAX_CUSTOMERS;
	int i = Q->front;
	while((i%MAX_CUSTOMERS)!=Q->rear)
	{
		printf("\n编号: %d\t到达时间：%d\t处理时间:%d\n",Q->customer[i].num,Q->customer[i].ArrivalTime,Q->customer[i].Duration);
		i++;
	}
	return;
}*/
